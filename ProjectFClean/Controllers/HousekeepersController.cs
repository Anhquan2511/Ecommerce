...
namespace ProjectFClean.Controllers
{
    ...
    {
        private ProjectFCleanEntities1 db = new ProjectFCleanEntities1();
        ...
        public ActionResult ChangeAvt(HttpPostedFileBase newAvatar) // HttpPostedFile
        {
            
            if (newAvatar != null)
            {
                String path = Server.MapPath("~/img/imgH/") + Path.GetExtension(newAvatar.FileName);
                newAvatar.SaveAs(path);
